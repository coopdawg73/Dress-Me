...
```tsx
// modify src/ui/StartScreen.test.tsx
// Add near the top of the file, alongside the existing imports (vi.mock calls
// are hoisted regardless of position, but this keeps the file readable):
import { getScores } from '../leaderboard/client'

vi.mock('../leaderboard/client', () => ({
  // Defaults to empty so the pre-existing tests below aren't affected by
  // this mock (a module-level vi.mock applies to every test in the file,
  // not just the one that needs it) - the new test below overrides the
  // resolved value for itself only, via mockResolvedValueOnce.
  getScores: vi.fn(async () => []),
}))

// Append this test after the existing describe('StartScreen', ...) block:
